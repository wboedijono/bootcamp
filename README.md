...
- To run filebeat and metricbeat playbooks, the above steps have fullfilled a few part of basic requirements. For examples, the ansible.cfg and hosts. But, the Filebeat-config.yml and metricbeat-config.yml are needed for beat agents to run accordindly. The beat config files need a few of information to be inserted (as shown below). Save them into /etc/ansible/files, so then they will get copied by the playbook into the VM which needs the agent. As mentioned in Playbook 3 section above as well, the beat  playbook will download the beat agent installer, install it before read the beat config, configure and launch it. (Below are the details of ELK Server for the agent where to fetch the stats or the metrics)  
  ```
