...
- To run other playbooks for filebeat and metricbeat, the above steps have fullfilled a few part of basic requirements. For examples, the ansible.cfg and hosts. The Filebeat-config.yml and metricbeat-config.yml are the beat agents config files those need to be adjusted (as shown below) then centralize them into /etc/ansible/files, so then they will get copied by the playbook into the VM which needs the agent. As mentioned in Playbook3 section above as well, the beat installer playbook will download the the agent installer, install it before read and configure it. (Below are the details of ELK Server for the agent where to fetch the stats or the metrics)  
  ```
